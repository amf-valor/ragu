...
describe('DeliveryLocalesComponent', () => {
  ...
  it(`GIVEN tax input WHEN user types any letter 
      THEN tax input should not accept and remain empty`, async () => {

    await userEvent.type(page.taxInput, 'as');
    expect(page.taxInput.textContent).toBe('');
  });

  it(`GIVEN tax input WHEN user types a number 
      THEN tax input should format the number to currency real`, async () => {
